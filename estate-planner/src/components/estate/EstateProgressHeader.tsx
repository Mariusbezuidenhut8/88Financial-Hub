...
import { estatePlannerSteps } from "../../data/estatePlannerSteps";
import type { EstatePlannerStepKey } from "../../data/estatePlannerSteps";

export interface EstateProgressHeaderProps {
  currentStepIndex:  number;
  completedStepKeys: EstatePlannerStepKey[];
  onStepClick:       (key: EstatePlannerStepKey) => void;
  onBackToDashboard: () => void;
}

export function EstateProgressHeader({
  currentStepIndex,
  completedStepKeys,
  onStepClick,
  onBackToDashboard,
}: EstateProgressHeaderProps) {
  const currentStep     = estatePlannerSteps[currentStepIndex];
  const totalSteps      = estatePlannerSteps.length;
  ...
  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <p className="text-sm font-medium text-slate-500">Estate Planner</p>
      ...
      {/* Step tabs */}
      <nav aria-label="Estate planner steps" className="mt-3">
        <ol className="flex items-center gap-1 overflow-x-auto pb-0.5">
          {estatePlannerSteps.map((step, i) => {
